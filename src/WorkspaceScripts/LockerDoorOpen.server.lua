...
script.Parent.Triggered:Connect(function()
	...
	local duration = .2 -- Duration of the rotation in seconds
	local elapsedTime = 0

	local startCFrame = pivot
	local targetCFrame

	local openSound = script.Parent:FindFirstChild("Open")  -- Reference to the Open sound
	...
	if not isOpen then
		...
		-- Open the door by rotating -75 degrees
		targetCFrame = pivot * CFrame.Angles(0, -rotationAngle, 0)
		...
		-- Close the door by rotating +75 degrees
		targetCFrame = pivot * CFrame.Angles(0, rotationAngle, 0)
		lockerDoor:SetAttribute("IsOpen", false)
